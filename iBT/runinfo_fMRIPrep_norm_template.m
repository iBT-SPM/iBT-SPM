% Integrated Brain Analysis Toolbox for SPM: 'runinfo_fMRIPrep_norm_template.m'
% Template version date: 2025-12-30
%
% Template of a file to indicate what you want done:
% This version is suitable for "norm-space" input data  
% that has already been pre-processed with fMRIPrep 
% (i.e. it is already spatially normalised to a group template).
% Please copy and edit to suit your needs.
% 
...
onsets = [0 40 80 120 160];   % task onsets (one task type, the remainder is REST), 0 is start of first included scan in pick list
durations = [20 20 20 20 20];          % task duration(s)
...
iBT.who.sub{sub}.sess{ses}.name='pseudoword';  %summary name for this task (independent of contrasts); if using fMRIPrep, be consistant here
...
iBT.who.sub{sub}.sess{ses}.rawloc=['/data/fmriprep/sub-55555/func']; % Location of raw images (or fMRIPrep output images)
...
iBT.who.sub{sub}.sess{ses}.tscans=202; %total number of scans in the data directory, -1= autodetect
iBT.who.sub{sub}.sess{ses}.nscans=190; %number of scans to include in analysis, -1= autodetect
iBT.who.sub{sub}.sess{ses}.pick=[11:200];  %range of scans to include in analysis (ignored for pre-processing; first image is 1)
...
% Wildcards suitable for fMRIPrep:
iBT.who.sub{sub}.sess{ses}.wild.input = ['sub-*_task-' iBT.who.sub{sub}.sess{ses}.name '_space-MNI152NLin6Asym_res-2*.nii.gz'];	% wildcard for input fMRIPrep func NIfTI images (includes the mask)
% The following inputAnat file is assumed located in ../anat/ relative to iBT.who.sub{sub}.sess{ses}.rawloc
iBT.who.sub{sub}.sess{ses}.wild.r_inputAnat =   ['']; % wildcard for input fMRIPrep NIfTI Anatomical image (own space)
iBT.who.sub{sub}.sess{ses}.wild.n_inputAnat =   ['sub-' iBT.who.sub{sub}.ID '_space-MNI152NLin6Asym_res-2_desc-preproc_T1w.nii.gz']; % wildcard for input fMRIPrep NIfTI Anatomical image (spatially normalised space)
iBT.who.sub{sub}.sess{ses}.wild.raw =   ['sub-*_task-' iBT.who.sub{sub}.sess{ses}.name '_space-MNI152NLin6Asym_res-2_desc-preproc_bold????.nii']; % wildcard for post-fslsplit NIfTI fMRI images
iBT.who.sub{sub}.sess{ses}.wild.r_ExplicitMask = [''];
iBT.who.sub{sub}.sess{ses}.wild.n_ExplicitMask = ['sub-*_task-' iBT.who.sub{sub}.sess{ses}.name '_space-MNI152NLin6Asym_res-2_desc-brain_mask????.nii']; % wildcard for post-fslsplit NIfTI fMRI within-brain mask  
iBT.who.sub{sub}.sess{ses}.wild.confounds_fmriprep = ['sub-*_task-' iBT.who.sub{sub}.sess{ses}.name '_desc-confounds_?????????s.tsv'];
 ...
 for i=1:num_rows,
   ...
   if num_cols >= 7,
	...
	iBT.who.sub{sub}.MasterSession=1; % Ensure a session is designated master even if not specified. If specified this will be changed below.
	 ...
	 % Filename wildcards suitable for fMRIPrep:
	iBT.who.sub{sub}.sess{ses}.wild.input = ['sub-*_task-' iBT.who.sub{sub}.sess{ses}.name '_space-MNI152NLin6Asym_res-2*.nii.gz'];	% wildcard for input fMRIPrep NIfTI images (includes the mask)
	% The following inputAnat file is assumed located in ../anat/ relative to iBT.who.sub{sub}.sess{ses}.rawloc
	iBT.who.sub{sub}.sess{ses}.wild.r_inputAnat =   ['']; % wildcard for input fMRIPrep NIfTI Anatomical image (own space)
	iBT.who.sub{sub}.sess{ses}.wild.n_inputAnat =   ['sub-' iBT.who.sub{sub}.ID '_space-MNI152NLin6Asym_res-2_desc-preproc_T1w.nii.gz']; % wildcard for input fMRIPrep NIfTI Anatomical image (spatially normalised space)
	iBT.who.sub{sub}.sess{ses}.wild.raw =   ['sub-*_task-' iBT.who.sub{sub}.sess{ses}.name '_space-MNI152NLin6Asym_res-2_desc-preproc_bold????.nii']; % wildcard for post-fslsplit NIfTI fMRI images  
	iBT.who.sub{sub}.sess{ses}.wild.r_ExplicitMask = [''];
	iBT.who.sub{sub}.sess{ses}.wild.n_ExplicitMask = ['sub-*_task-' iBT.who.sub{sub}.sess{ses}.name '_space-MNI152NLin6Asym_res-2_desc-brain_mask????.nii'];	% wildcard for our NIfTI fMRI within-brain mask  
	iBT.who.sub{sub}.sess{ses}.wild.confounds_fmriprep = ['sub-*_task-' iBT.who.sub{sub}.sess{ses}.name '_desc-confounds_?????????s.tsv'];
	iBT.who.sub{sub}.sess{ses}.wild.slice =   iBT.who.sub{sub}.sess{ses}.wild.raw; % Because fMRIPrep has already done the slice timing adjustment
	iBT.who.sub{sub}.sess{ses}.wild.realign = iBT.who.sub{sub}.sess{ses}.wild.raw; % Because fMRIPrep has already done the realignment
	iBT.who.sub{sub}.sess{ses}.wild.r = cat(2, 's', iBT.who.sub{sub}.sess{ses}.wild.raw);	% wildcard for non-normalised images to analyse
...
iBT.what.pre.do=1;                 % preprocess data: 1(yes) 0(no)
   iBT.what.pre.fmriprep.done=1;  % Has the "raw" data been processed with fmriprep?: 1(yes) 0(no)
   ...
   iBT.what.pre.DICOMconvert = 0;  % 0, 1 or 2 (or negative if already done).
   ...
   iBT.what.pre.SliceTiming = 0;   % 1(yes) 0(no) -1(already done)
   ...
   iBT.what.pre.WriteRealign = 0;  % write images for own-space analysis: 1(yes) 0(no) -1(already done)
   ...
   iBT.what.pre.norm.do = 0;   % perform spatial normalisation to a template: 0(no), 1(yes, nonlinear warp), 2(yes, affine only), 3(yes, near rigid-body only), -1 or -2 or -3 (already done)
...
iBT.what.stats.do=1;     % analyse data: 1(yes) 0(no)
	...
	iBT.what.stats.own_space.do = 0;   % Analyse subject's own-space data 
	    ...
	    iBT.what.stats.r_withinBrainMask = 'Explicit'; % Set this to 'iBrain', 'BET', 'SPM' or 'Explicit' depending upon type of own-space mask you want to use.
	...
	iBT.what.stats.norm_space.do = 1;  % Analyse subjects' spatially normalised data
	    iBT.what.stats.n_withinBrainMask = 'Explicit'; % Set this to 'iBrain', 'BET', 'SPM' or 'Explicit' depending upon type of norm-space mask you want to use.
...
%        iBT.what.disp.orient.slices = {64.5:-3:-64.5 94.5:-3:-94.5 -94.5:3:94.5};  % slice range corresponding to original own-space slices
	...
	iBT.what.disp.threshold.type =     {'p'       'FDRc'       't'       'FWE'}; % Type of threshold, as follows: 'p' ,'FWE' , or
	  ...
	  iBT.what.disp.threshold.thresh = {0.001     [0.001 0.05] 0         0.05}; % lower threshold for magnitude of statistics (0 for no threshold; -1 for type = 'li' described above)
...
%% Analysis/statistics
       ...
       iBT.what.stats.number_dirs = 0; % prepend NN_ to subfolder names, where NN is the session count starting from 01.
...
%%  Display
	...
	iBT.what.disp.show.procVoxelSize = 1; % Display processed volume voxel sizes % 1(yes) 0(no, default)
