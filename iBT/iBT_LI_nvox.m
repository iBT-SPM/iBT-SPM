function [l,r,li,nvox,thresh,maxn]= iBT_LI_nvox(left, right, act, step)
% Calculate li (and thresh etc.) for a range of nvox (number of voxels
%  above thresh) with nvox ganularity = step
% FORMAT [l,r,li,nvox,thresh,maxn]= iBT_LI_nvox(left, right, act, step)
% where
...
%  step = the granularity of nvox for which to generate results
% 
...
% 2010-08-09: (dfa) supported images where there are no positive voxels at any threshold..
% 2009-11-09: (dfa) added maxn return.
...
maxn = sum(sum(sum((all>0).*(act>0.00001))))   % this is the max no. of voxels that will be activated

if maxn > 0
  act_roi = act(find(all > 0));
  sort_act_roi = sort(act_roi);
  % The flip function works on columns. sort_act_roi data is 
  % stored as a row hence need for transverse (').
  sort_act_roi = fliplr(sort_act_roi');

  nsteps=floor(maxn/step);
 
  for n=1:nsteps,

	nvox(n)=n*step;
    	thresh(n)=sort_act_roi(nvox(n)+1);
