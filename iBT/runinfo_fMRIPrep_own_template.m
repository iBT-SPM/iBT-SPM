% Integrated Brain Analysis Toolbox for SPM: 'runinfo_fMRIPrep_own_template.m'
...
% (i.e. individual-subject space) that has already
% been pre-processed with fMRIPrep.
...
iBT.who.sub{sub}.sess{ses}.wild.input = ['sub-*_task-' iBT.who.sub{sub}.sess{ses}.name '_space-T1w*.nii.gz']; % wildcard for input fMRIPrep func NIfTI images (includes the mask)
% The following inputAnat file is assumed located in ../anat/ relative to iBT.who.sub{sub}.sess{ses}.rawloc
iBT.who.sub{sub}.sess{ses}.wild.r_inputAnat =   ['sub-' iBT.who.sub{sub}.ID '_desc-preproc_T1w.nii.gz']; % wildcard for input fMRIPrep NIfTI Anatomical image (own space)
iBT.who.sub{sub}.sess{ses}.wild.n_inputAnat =   ['']; % wildcard for input fMRIPrep NIfTI Anatomical image (spatially normalised space)
iBT.who.sub{sub}.sess{ses}.wild.raw =   ['sub-*_task-' iBT.who.sub{sub}.sess{ses}.name '_space-T1w_desc-preproc_bold????.nii'];	% wildcard for post-fslsplit NIfTI fMRI images
iBT.who.sub{sub}.sess{ses}.wild.r_ExplicitMask = ['sub-*_task-' iBT.who.sub{sub}.sess{ses}.name '_space-T1w_desc-brain_mask????.nii'];	% wildcard for post-fslsplit NIfTI fMRI within-brain mask
iBT.who.sub{sub}.sess{ses}.wild.n_ExplicitMask = [''];
...
iBT.what.stats.do=1;     % analyse data: 1(yes) 0(no)
	...
	iBT.what.stats.own_space.do = 1;   % Analyse subject's own-space data 
	...
	iBT.what.stats.norm_space.do = 0;  % Analyse subjects' spatially normalised data
